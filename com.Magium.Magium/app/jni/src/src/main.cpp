...
void setupGameTextScene() {
    ...
    std::shared_ptr<RawText> contentText =
        std::make_shared<RawText>(g_width * sidePadding, g_height * topPadding, "placeholder", .82f, COLOR_GAME_TEXT_STORY_TEXT,
                                  g_fontMainText, g_width * (1.f - sidePadding * 2.f));
    ...
    std::shared_ptr<Button> mainMenuButton = std::make_shared<Button>(Helper::swiths(0, .05f, .3f, .05f), COLOR_GAME_TEXT_MENU_BUTTON,
                                                                      COLOR_GAME_TEXT_MENU_BUTTON_TEXT, "MAIN MENU", .8f, []() {
                                                                          mainMenuScene->enable();
                                                                          gameTextScene->disable();
                                                                      });
    gameTextScene->add(mainMenuButton);

    std::shared_ptr<Button> statsButton = std::make_shared<Button>(Helper::swiths(.8f, .05f, .2f, .05f), COLOR_GAME_TEXT_MENU_BUTTON,
                                                                   COLOR_GAME_TEXT_MENU_BUTTON_TEXT, "STATS", .8f, []() {
    ...
    std::shared_ptr<RawImage> topHider =
        std::make_shared<RawImage>(SDL_FRect{0, -100.f, g_width, topBar->rect().h + 100.f}, COLOR_GAME_TEXT_BOTTOM_BACKGROUND);
...
void setupMainMenuScene() {
    ...
    std::shared_ptr<RawImage> mainMenuBackground =
        std::make_shared<RawImage>(SDL_FRect{0, 0, g_width, g_height}, COLOR_MAIN_MENU_BACKGROUND);
    mainMenuScene->add(mainMenuBackground);

    SDL_FRect mainMenuButtonsPos = {g_width / 2.f, g_height * (MAIN_MENU_BUTTON_DISTANCE + MAIN_MENU_BUTTON_PADDING),
                                    g_width * MAIN_MENU_BUTTON_WIDTH, g_height * MAIN_MENU_BUTTON_HEIGHT};
    ...
    mainMenuButtonsPos.y += MAIN_MENU_BUTTON_DISTANCE * g_height;
    std::shared_ptr<Button> newGameButton = std::make_shared<Button>(mainMenuButtonsPos, COLOR_MAIN_MENU_BUTTON,
                                                                     COLOR_MAIN_MENU_BUTTON_TEXT, "New game / Restart Book", .8f, []() {
                                                                     });
    mainMenuScene->add(newGameButton);

    mainMenuButtonsPos.y += MAIN_MENU_BUTTON_DISTANCE * g_height;
    std::shared_ptr<Button> loadFromLastCheckpointButton = std::make_shared<Button>(
        mainMenuButtonsPos, COLOR_MAIN_MENU_BUTTON, COLOR_MAIN_MENU_BUTTON_TEXT, "Load from last checkpoint", .8f, []() {
	...
	//std::shared_ptr<UIScene> saveLoadMenu = std::make_shared<UIScene>();
	//std::vector<std::shared_ptr<SlotsScene>> saveSlotsScenes(5, std::make_shared<UIScene>());

    mainMenuButtonsPos.y += MAIN_MENU_BUTTON_DISTANCE * g_height;
    std::shared_ptr<Button> aboutButton =
        std::make_shared<Button>(mainMenuButtonsPos, COLOR_MAIN_MENU_BUTTON, COLOR_MAIN_MENU_BUTTON_TEXT, "About", .8f, []() {
...
void setupStatsMenuScene() {
    ...
    std::shared_ptr<RawImage> statsMenuBackground =
        std::make_shared<RawImage>(SDL_FRect{0, 0, g_width, g_height}, COLOR_STATS_MENU_BACKGROUND);
    statsMenuScene->add(statsMenuBackground);

    std::shared_ptr<RawText> availablePointsText = std::make_shared<RawText>(.5f * g_width, .15f * g_height, "Available points: 26", .82f,
                                                                             COLOR_STATS_MENU_AVAILABLE_POINTS_TEXT, g_fontMainText);
    ...
    std::shared_ptr<Button> statsMenuCancelButton = std::make_shared<Button>(
        statsMenuButtonPos, COLOR_STATS_MENU_BUTTON, COLOR_STATS_MENU_BUTTON_TEXT, "Cancel changes", .8f, [stats]() {
    ...
    std::shared_ptr<RawText> statsText =
        std::make_shared<RawText>(.5f * g_width, .07f * g_height, "STATS", .9f, SDL_Color{0, 0, 0, 255}, g_fontBold);
